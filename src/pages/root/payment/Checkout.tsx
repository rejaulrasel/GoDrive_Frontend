...
const Checkout: React.FC<{
  ...
  booking: TBookingResponse[] | null;
}> = ({ bookingId, booking }) => {
  const [clientSecret, setClientSecret] = useState<string | null>(null);
  console.log("1st:", clientSecret);
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 401) {
            ...
            throw new Error(
              "booking is not successful yet or the payment is already completed"
            );
          } else if (response.status === 400) {
            throw new Error(
              "You cannot make payments on someone else's booking. Not even GF/BF"
            );
          } else {
        ...
        // console.log("2nd clientSecret:", clientSecret);
  ...
  const handlePayment = async (e: FormEvent) => {
    ...
    console.log("before payment error, clientsecret is=", clientSecret);
        ...
        const EMAIL_PARAMS: TNotificationEmail = {
          ...
          description: `We have successfully received your payment of USD ${
            booking ? booking[0].totalCost : null
                    ...
                    Rent Period: ${
                      booking
                        ? booking[0].totalCost / booking[0].car.pricePerHour
                        : null
                    } Hours
