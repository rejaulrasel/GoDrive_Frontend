...
const VehicleDetails: FC = () => {
  ...
  const moreItemExceptThisOne: TVehicleResponse[] = data?.data?.filter(
    (data: { _id: string }) => data._id !== item?.data?._id
  );
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        const EMAIL_PARAMS: TNotificationEmail = {
          ...
          description: `Thank you for choosing GoDrive! We have received your booking and it is currently pending verification.
        ...
        <div className="grid items-start grid-cols-1 lg:grid-cols-5 gap-12 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] p-6 rounded-lg">
          ...
          <div className="lg:col-span-2">
            <h2 className="text-2xl font-extrabold text-gray-800">
              {item?.data?.name} {item?.data?.color}
            </h2>
            ...
            <div className="mt-5">
              <h1 className="font-semibold">
                Location: {item?.data?.location}, Bangladesh
              </h1>
              <h1 className="font-semibold">
                Fuel Type: {item?.data?.fuelType}
              </h1>
              <div className="flex gap-1 mt-2">
                <p className="font-semibold">Features:</p>
                <div className=" flex items-center gap-2 flex-wrap">
                  {item?.data?.features?.map(
                    (feature: string, indx: number) => {
                      return (
                        <span
                          key={indx}
                          className="bg-gray-200 text-sm rounded-sm px-2"
                        >
                          {feature}
                        </span>
                      );
            ...
            {item?.data?.status === "available" ? (
              <div className="flex flex-wrap gap-4 mt-8">
                <button
                  ...
                  type="button"
                  className="w-full  px-4 py-2.5 border bg-rose-600 hover:bg-rose-700 text-white text-sm font-semibold rounded"
                >
                  Book Now
                </button>
              </div>
              ...
              <button
                type="button"
                className="w-full mt-20 px-4 py-2.5 border border-rose-600 bg-transparent hover:bg-gray-50 text-rose-800 text-sm font-semibold cursor-not-allowed rounded"
              >
                Vehicle Unavailable
              </button>
            )}
      ...
      <div className="mx-auto flex w-72 items-center justify-center">
        ...
        >
          <div
            ...
            className={`absolute rounded-lg bg-white drop-shadow-2xl w-[95%] md:w-[60%] ${
              openModal
          ...
          >
            <form
              onSubmit={handleSubmit(handleCreateNewBooking)}
            ...
            >
              ...
              <h1 className="pb-8 text-3xl backdrop-blur-sm">
                Book {item?.data?.name} {item?.data?.fuelType}{" "}
                {item?.data?.color}
              </h1>
              ...
              <button
                type="submit"
                className="relative py-2.5 px-5 rounded-md text-white mt-6 bg-rose-600 drop-shadow-lg hover:bg-rose-700"
              >
                Book
              </button>
            </form>
