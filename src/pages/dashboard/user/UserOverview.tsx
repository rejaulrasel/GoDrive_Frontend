...
const UserOverview: React.FC = () => {
  ...
  const successCount = bookingHistory?.data?.filter(
    (booking: TBookingResponse) => booking.status === "succeed"
  ).length;
  const cancelCount = bookingHistory?.data?.filter(
    (booking: TBookingResponse) => booking.status === "canceled"
  ).length;
        ...
        <Card>
          <CardContent>
            <Typography variant="h6">
              Booking Success vs Cancellation Rate
            </Typography>
            <PieChart
              series={[
                {
