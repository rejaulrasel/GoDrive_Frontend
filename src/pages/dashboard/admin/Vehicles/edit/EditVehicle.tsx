...
type createVehicleSchema = TypeOf<
  typeof VehicleValidation.createVehicleValidationSchema
>;
...
const EditVehicle: React.FC = () => {
  ...
  >({ name: "Error" });
  const [initialValues, setInitialValues] = useState<TVehicleResponse | null>(
    null
  );
    ...
    const fetchVehicle = async () => {
      try {
        const vehicle = await axios.get(
          `http://localhost:5000/api/cars/${_id}`
        );
        setInitialValues(vehicle?.data?.data);
        setShowFileName({
          name: `${vehicle?.data?.data?.photo.slice(0, 35)}...`,
        });
    ...
    if (`${initialValues?.photo.slice(0, 35)}...` !== showFileName?.name) {
      ...
      if (imgBbResponse.success) {
        await patchVehicle({
          ...
          .then(() => {
            toast.success(`${initialValues?.name} is Updated🥱`, {
              id: toastId,
            });
  ...
  return (
    <div>
      ...
      <form
        className="mt-10 mx-5 space-y-5"
      ...
      >
        <div className="flex justify-between gap-5">
          <div className="flex-1">
            ...
            />
            {errors.name && (
              <small className=" ml-1 text-rose-600">
                {errors.name?.message}
              </small>
            )}
          </div>
        ...
        <div className="">
          ...
          <div>
            <label
              htmlFor="type2-1"
              className="flex max-w-[380px] md:w-[380px]"
            >
              <div className="w-fit whitespace-nowrap  bg-rose-500  px-2 py-1 text-sm text-white">
                Choose File
              </div>
              <div className=" flex w-full max-w-[380px] items-center  border-b-[2px] border-rose-500 px-2 text-sm font-medium text-gray-400">
                {showFileName!.name.length > 35
                  ? `${showFileName!.name.slice(0, 35)}...`
                  : showFileName!.name}
              </div>
            </label>
            <input
