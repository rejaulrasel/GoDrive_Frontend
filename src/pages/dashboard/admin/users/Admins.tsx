...
const Admins = () => {
  ...
  const {
    data: dataWithLoggedUser,
    isError,
    isLoading,
  } = useGetRoleBaseUsersQuery([{ role: "admin" }], { skip: !currentUser });

  const admins = dataWithLoggedUser?.data.filter(
    (user: TFullUser) => user.email !== currentUser?.user
  );
  ...
  return (
    <div data-aos="fade-left" className="">
      ...
      <div className="flex flex-col w-full  mx-auto">
        ...
        {admins?.map((user: TFullUser, indx: number) => (
          <UsersCard key={indx} {...user} />
        ))}
      </div>
    </div>
  );
