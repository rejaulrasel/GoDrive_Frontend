...
const BookingCard: React.FC<TBookingCardProps> = ({
  setClickedItem,
  booking,
}) => {
      ...
      if (result.isConfirmed) {
          ...
          if (returnRes?.data?.success) {
            Swal.fire({
              ...
              if (result.isConfirmed) {
                ...
                const paymentUrl =
                  baseUrl + `/user/booking/payout/${booking._id}`;
        ...
        if (res?.data?.success) {
          ...
          Swal.fire({
            ...
            text: `Order has been ${
              action === "approve" ? "Approved" : "Canceled"
            }`,
            icon: "success",
          });
        }
      ...
      <div className="flex">
        ...
        <div className="flex-1 py-2 px-4 border-b border-gray-300 space-x-2">
          <button
            ...
            className="border bg-gray-200 hover:bg-gray-300 transition-all hover:scale-105 px-1 rounded-md"
          >
            View
          </button>
          ...
          {booking.status !== "canceled" && (
            <button
              onClick={() => {
                handleBookingStatus(
                  booking?.status === "pending" ? "approve" : "return"
                );
              }}
          ...
          {booking?.status === "pending" && (
            <button
              onClick={() => handleBookingStatus("cancel")}
              className="border bg-rose-600 text-white rounded-md hover:bg-rose-700 transition-all hover:scale-105 px-1 "
            >
              Cancel
            </button>
          )}
          ...
          {booking?.status === "canceled" && (
            <button
              onClick={handleDeleteCanceledBooking}
              className="border bg-rose-600 text-white rounded-md hover:bg-rose-700 transition-all hover:scale-105 px-1 "
            >
              Delete
            </button>
