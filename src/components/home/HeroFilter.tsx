...
const HeroFilter = () => {
  ...
  const handleFormSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    const location =
      data?.location?.charAt(0)?.toLowerCase() + data?.location?.slice(1) || "";
    const color =
      data?.color?.charAt(0)?.toLowerCase() + data?.color?.slice(1) || "";

    try {
      const res = await axios.get(
        `https://car-rental-reservation-system-nine.vercel.app/api/cars`,
        {
          params: {
            location,
            color,
  ...
  return (
    <div
      data-aos="zoom-out"
      className="w-96 hidden md:flex flex-col p-7 bg-rose-600 bg-opacity-20 rounded-md -mt-40"
    >
      <form
      ...
      >
        ...
        <div className="!mt-5">
          {!data?.length ? (
            <button
              type="submit"
              className="py-2 w-full text-center px-2 bg-rose-600 hover:bg-rose-700 transition-all rounded-md"
            >
              Search
            </button>
          ) : (
            <button
              type="button"
              onClick={handleNavigateToVehicleRoute}
              className="py-2 w-full text-center px-2 bg-rose-600 hover:bg-rose-700 transition-all rounded-md"
            >
              Browse {data?.length} Vehicles
            </button>
          )}
        </div>
      </form>
